...
const SignIn = () => {
    ...
    const sendOTP = () => {
        ...
        const confirmationResult = window.confirmationResult;
            confirmationResult.confirm(otp.current.value).then((result) => {
                alert("signed in")
              }).catch((error) => {
    ...
    const InputRequst = () => {
        ...
        for(let j = 0; j < users.length; j++) {
            if(users[j].email !== email.current.value || users[j].phone !== phone.current.value){
                bool = true;
                console.log(users[j]);
            }
            else {
    ...
    return(
        <Paper sx={styles.signUpPage}>
            <Card sx={styles.cardSection}>
                <CardMedia 
                    component="img"
                    src="https://pngimg.com/uploads/netflix/netflix_PNG15.png" alt="" 
                    sx={styles.logo}
                />
                <Box>
                    <Box>
                ...
                <Card 
                    sx={bool ? styles.signUpActive : styles.signUp} 
                >
                    <Box>
                        <TextField error label="email..." sx={styles.input} type="email" inputRef={email}/>
                    </Box>
                    <Box>
                        <TextField error label="name..." sx={styles.input} type="text" inputRef={name}/>
                    </Box>
                    <Box>
                        <TextField error label="password..." sx={styles.input} type="password" inputRef={pass}/>
                    </Box>
            ...
            <div id="recaptchaContainer" style={{ position: "absolute", bottom: "1em", right: "1em" }}></div>
        </Paper>
    )
}
